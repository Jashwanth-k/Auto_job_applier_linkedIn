...
def apply_filters() -> None:
    ...
    try:
        buffer(3)
        recommended_wait = 0 #if click_gap < 1 else 0

        wait.until(EC.presence_of_element_located((By.XPATH, '//button[normalize-space()="All filters"]'))).click()
        buffer(recommended_wait)

        if sort_by:
            clicks_sucess = clicks_sucess and wait_span_click(driver, sort_by, 1)
        if date_posted:
            clicks_sucess = clicks_sucess and wait_span_click(driver, date_posted, 1)
        buffer(recommended_wait)
        ...
        if salary:
            clicks_sucess = clicks_sucess and wait_span_click(driver, salary, 1)
        buffer(recommended_wait)
...
def get_job_description(
    ...
    try:
        ...
        noOfEmployeesAssiciated = find_by_class(driver , "jobs-company__inline-information", 5.0, -1).text or '0'
        companyName = find_by_class(driver, "job-details-jobs-unified-top-card__company-name").text
        ...
        if not skip:
            ...
            if current_experience > -1 and experience_required > current_experience + found_masters:
                ...
                skip = True
        noOfEmployeesAssiciated = int((noOfEmployeesAssiciated.split('-')[0]).split(' ')[0].replace(',', ''))
        if noOfEmployeesAssiciated < min_noof_associated_employees:
            skipMessage = f'\n{companyName} Does not have enough employees "{noOfEmployeesAssiciated}". Skipping this job!\n'
            skipReason = "Required No of Employees is low"
            skip = True
    except Exception as e:
            ...
            while current_count < switch_number:
                ...
                for job in job_listings:
                    try: 
                        ...
                        print_lg("Failed to apply Job!",e)
